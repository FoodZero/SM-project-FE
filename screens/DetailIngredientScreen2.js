...
const DetailIngrediant = () => {
  ...
  const name = route.params?.ingredientText;
  ...
  const handleClose = () => {
    ...
    navigation.navigate('AddIngredientScreen', { AccessToken: AccessToken, id:refrigeratorId });
  };
  ...
  const SaveFood = async () => {
    ...
    // Convert the selected date to ISO format (if needed) but use only the date part
    const expire = selectedDate 
      ? new Date(selectedDate).toISOString().split('T')[0]  // Ensure YYYY-MM-DD format
      : new Date().toISOString().split('T')[0];  // Default to today's date if no date is selected
